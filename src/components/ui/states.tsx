...
export function PageSkeleton({ cards = 4, rows = 3 }: { cards?: number; rows?: number }) {
  return (
    <div role="status" aria-label="Caricamento in corso">
      ...
      <span className="sr-only" aria-live="polite">Caricamento contenuto della pagina…</span>
    </div>
  );
}
...
export function ErrorState({
  ...
  return (
    <div ref={containerRef} tabIndex={-1} role="alert" className="rounded-3xl border border-rose-200 bg-rose-50 px-6 py-8 text-center text-rose-950 shadow-sm shadow-rose-950/5 outline-none">
